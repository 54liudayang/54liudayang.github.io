...
void setup(){
  ...
  agentPos[0] = new Vec2(random(850),random(650));
  agentPos[1] = new Vec2(random(850),random(650));
  agentPos[2] = new Vec2(random(850),random(650));
  agentPos[3] = new Vec2(random(850),random(650));
  agentPos[4] = new Vec2(random(850),random(650));
  goalPos[0] = new Vec2(random(850),random(650));
  goalPos[1] = new Vec2(random(850),random(650));
  goalPos[2] = new Vec2(random(850),random(650));
  goalPos[3] = new Vec2(random(850),random(650));
  goalPos[4] = new Vec2(random(850),random(650));
...
float computeTTC(Vec2 pos1, Vec2 vel1, float radius1, Vec2 pos2, Vec2 vel2, float radius2){
  ...
  float t = rayCircleIntersectTime (pos2, R, pos1, relativeVelocity);
  return t;
}
...
Vec2 computeAgentForces(int id){
  ...
  acc.add((goalVel.minus(agentVel[id])).times(k_goal));
  
  if (goalVel.length() < 5) return acc;
  
  for (int j = 0; j < numAgents; j++){
    if (j == id) continue;
    float t = computeTTC(agentPos[id],agentVel[id], agentRad, agentPos[j],agentVel[j], agentRad);
    Vec2 futurePos_id = agentPos[id].plus(agentVel[id].times(t));
    Vec2 futurePos_j = agentPos[j].plus(agentVel[j].times(t));
    Vec2 avoidDir = futurePos_id.minus(futurePos_j).normalized();
    Vec2 avoidForce = avoidDir.times(1/t);
    if (t > 0){
